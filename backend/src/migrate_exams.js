...
// Fix the DATABASE_URL in the .env if it has weird prefix in the file
...
CREATE TYPE IF NOT EXISTS exam_type AS ENUM (
  ...
  'PROGRESS',     -- based on current user progress
  'IELTS',        -- IELTS standard (no user context needed)
  'PRACTICE'      -- like PROGRESS but no XP awarded
);

CREATE TYPE IF NOT EXISTS exam_status AS ENUM (
  'GENERATING',   -- LLM is creating questions
  'READY',        -- questions generated, user can start
  'IN_PROGRESS',  -- user is taking the exam
  'SUBMITTED',    -- user finished, awaiting evaluation
  'EVALUATING',   -- background evaluation running
  'EVALUATED',    -- results ready
...
CREATE TYPE IF NOT EXISTS question_section AS ENUM ('LISTENING', 'SPEAKING');
CREATE TYPE IF NOT EXISTS question_item_type AS ENUM ('WORD', 'PHRASE');
...
async function run() {
  try {
    // A small hack since 'CREATE TYPE IF NOT EXISTS' is not supported for ENUMs in older PG versions, 
    // we'll run it in a DO block to ignore errors if it exists.
    const preSql = `
      DO $$
      BEGIN
        ...
        END IF;
        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'question_section') THEN
          CREATE TYPE question_section AS ENUM ('LISTENING', 'SPEAKING');
        END IF;
        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'question_item_type') THEN
    ...
    // Remove the CREATE TYPE lines from main sql block as we handled them
    let cleanSql = sql.replace(/CREATE TYPE IF NOT EXISTS [^;]+;/g, '');
    ...
    // Append to doc files
    fs.appendFileSync(path.join(__dirname, 'docs', 'tables.sql'), '\\n\\n-- Dynamic Exam System Tables\\n' + cleanSql.split('ALTER TABLE notifications')[0]);
    fs.appendFileSync(path.join(__dirname, 'docs', 'triggers.sql'), '\\n\\n-- Dynamic Exam System Triggers\\n' + sql.match(/CREATE OR REPLACE FUNCTION trg_xp_notification()[\\s\\S]*?\\$\\$ LANGUAGE plpgsql;/)[0]);
    fs.appendFileSync(path.join(__dirname, 'docs', 'views.sql'), '\\n\\n-- Dynamic Exam System Views\\n' + sql.match(/CREATE OR REPLACE VIEW vw_user_stats[\\s\\S]*?CREATE OR REPLACE VIEW vw_user_activity_dates[\\s\\S]*?;/)[0]);
    console.log("Appended to doc files successfully.");
